(function () {
  ...
  var minBondDistance = 0.35;
  var maxBondDistance = 2.65;
  var bondRadiusScale = 1.25;
  var spatialBondThreshold = 96;
  ...
  function StructureViewer(canvas, options) {
    ...
    this.projectedAtoms = [];
    this.pixelRatio = 1;
    this.bondCache = new WeakMap();
    this.centeredAtomCache = new WeakMap();
    ...
    this.selected =
      this.frame && selectedIndices.length
        ? selectedIndices
            ...
            .filter(Boolean)
        : [];
    this.bonds = this.frame ? this.bondsForFrame(this.frame) : [];
    if (settings.fit !== false) this.fit(settings.referenceAtoms || (this.frame ? this.frame.atoms : null));
    this.draw();
    if (hadSelection || this.selected.length > 0) this.emitSelection();
  };

  StructureViewer.prototype.bondsForFrame = function (frame) {
    if (!frame || !frame.atoms) return [];
    var cached = this.bondCache.get(frame);
    if (cached) return cached;
    var bonds = inferBonds(frame.atoms);
    this.bondCache.set(frame, bonds);
    return bonds;
  };

  StructureViewer.prototype.centeredAtomsForFrame = function (frame) {
    if (!frame || !frame.atoms) return [];
    var cached = this.centeredAtomCache.get(frame);
    if (cached) return cached;
    var centered = centerAtoms(frame.atoms, this.boundsForAtoms(frame.atoms));
    this.centeredAtomCache.set(frame, centered);
    return centered;
  };

  StructureViewer.prototype.boundsForAtoms = function (atoms) {
    if (!atoms || !atoms.length) return null;
    var cached = this.boundsCache.get(atoms);
    if (cached) return cached;
    var box = bounds(atoms);
    this.boundsCache.set(atoms, box);
    return box;
  };
  ...
  StructureViewer.prototype.fit = function (atoms) {
    ...
    var box = this.boundsForAtoms(fitAtoms);
  ...
  StructureViewer.prototype.draw = function () {
    ...
    if (!this.frame || !this.frame.atoms.length) {
      ...
      ctx.fillText(uiText("viewer.openHint", "打开 xyz / inp / out / restart 文件"), w / 2, h / 2);
      this.projectedAtoms = [];
      return;
    }

    var centered = this.centeredAtomsForFrame(this.frame);
    var projected = projectAtoms(centered, this.orientation, {
      width: w,
      height: h,
      pan: this.pan,
      zoom: this.zoom,
  ...
  StructureViewer.prototype.drawCell = function (ctx, centered, w, h) {
    ...
    var screen = projectCellPoints(this.frame.cell, this.orientation, {
      width: w,
      height: h,
      pan: this.pan,
      zoom: this.zoom,
    });
    drawCellEdges(ctx, screen, this.pixelRatio);
  };
  ...
  StructureViewer.prototype.drawBonds = function (ctx, projected) {
    ...
    drawBondOutlines(ctx, this.bonds, projected, width + outline);
    drawBondColorSegments(ctx, this.bonds, projected, width);
    ctx.restore();
  };

  StructureViewer.prototype.drawAtoms = function (ctx, projected) {
    var sorted = projected.slice().sort(function (a, b) { return a.z - b.z; });
    var selectedLookup = selectedIndexLookup(this.selected);
    var self = this;
    sorted.forEach(function (atom) {
      var selected = Boolean(selectedLookup[atom.index]);
      var style = atomColorStyle(atom.element);
      drawAtomSphere(ctx, atom, style, selected, self.pixelRatio);

      // Element label — clean, no stroke/halo
      if (selected || self.showLabels) {
        drawAtomElementLabel(ctx, atom, style, self.pixelRatio);
      }

      if (selected) {
        drawAtomIndexLabel(ctx, atom, self.pixelRatio);
      }
    });
  };
  ...
  function projectAtoms(centered, orientation, settings) {
    return centered.map(function (atom) {
      ...
      return {
        ...
        y: rot.y,
        z: rot.z,
        screenX: settings.width / 2 + settings.pan.x + rot.x * settings.zoom,
        screenY: settings.height / 2 + settings.pan.y - rot.y * settings.zoom,
        radius: atomRadius(atom.source.element, settings.zoom) * settings.atomScale * settings.pixelRatio,
        fog: 0,
      };
    });
  }

  function projectCellPoints(cell, orientation, settings) {
    var points = cellCornerPoints(cell);
    var boxCenter = average(points);
    return points.map(function (point) {
      var shifted = { x: point.x - boxCenter.x, y: point.y - boxCenter.y, z: point.z - boxCenter.z };
      var rot = rotatePoint(shifted, orientation);
      return {
        x: settings.width / 2 + settings.pan.x + rot.x * settings.zoom,
        y: settings.height / 2 + settings.pan.y - rot.y * settings.zoom,
      };
    });
  }

  function cellCornerPoints(cell) {
    ...
    return [
      ...
      add(vectors[0], vectors[2]),
      add(vectors[1], vectors[2]),
      add(add(vectors[0], vectors[1]), vectors[2]),
    ];
  }

  function drawCellEdges(ctx, screen, ratio) {
    ...
    edges.forEach(function (edge) {
      ctx.beginPath();
      ctx.moveTo(screen[edge[0]].x, screen[edge[0]].y);
      ctx.lineTo(screen[edge[1]].x, screen[edge[1]].y);
      ctx.stroke();
    });
    ctx.restore();
  }

  function drawBondOutlines(ctx, bonds, projected, width) {
  ...
  function drawBondColorSegments(ctx, bonds, projected, width) {
    bonds.forEach(function (pair) {
      var p1 = projected[pair[0]];
      var p2 = projected[pair[1]];
      if (!p1 || !p2) return;
      var middle = bondMidpoint(p1, p2);
      drawBondSegment(ctx, p1.screenX, p1.screenY, middle.x, middle.y, colors[p1.element] || "#aeb8c4", width);
      drawBondSegment(ctx, p2.screenX, p2.screenY, middle.x, middle.y, colors[p2.element] || "#aeb8c4", width);
    });
  }

  function bondMidpoint(p1, p2) {
    return {
      x: (p1.screenX + p2.screenX) / 2,
      y: (p1.screenY + p2.screenY) / 2,
    };
  }

  function drawAtomSphere(ctx, atom, style, selected, ratio) {
    ...
    gradient.addColorStop(0.4, style.base);
    ...
    ctx.beginPath();
    ctx.arc(atom.screenX, atom.screenY, atom.radius, 0, Math.PI * 2);
    ctx.lineWidth = selected ? 3.2 * ratio : 1.2 * ratio;
    ctx.strokeStyle = selected ? "#f1c84b" : "rgba(27, 33, 40, 0.62)";
    ctx.stroke();
  }

  function drawAtomElementLabel(ctx, atom, style, ratio) {
    var fontSize = Math.max(10, Math.min(14, atom.radius * 0.82 / ratio)) * ratio;
  ...
  function drawAtomIndexLabel(ctx, atom, ratio) {
    ctx.fillStyle = "#f9fbfd";
    ctx.font = (11 * ratio) + "px Segoe UI, Microsoft YaHei, sans-serif";
    ctx.textBaseline = "alphabetic";
    ctx.fillText("#" + atom.index, atom.screenX, atom.screenY - atom.radius - 8 * ratio);
  }

  function atomColorStyle(element) {
    var color = colors[element] || "#9aa7b5";
    var cached = atomColorStyleCache[color];
    if (cached) return cached;
    cached = {
      base: color,
      light: lighten(color, 1.0),
      dark: darken(color, 1.0),
      labelColor: readableTextColor(color),
    };
    atomColorStyleCache[color] = cached;
    return cached;
  }
  ...
  function readableTextColor(color) {
    ...
    return lum > 0.55 ? "#141820" : "#f9fbfd";
  }

  function inferBonds(atoms) {
    if (atoms.length < spatialBondThreshold) return inferBondsBruteForce(atoms);
    return inferBondsSpatial(atoms);
  }

  function inferBondsBruteForce(atoms) {
    var bonds = [];
    for (var i = 0; i < atoms.length; i++) {
      for (var j = i + 1; j < atoms.length; j++) {
        if (isBonded(atoms[i], atoms[j])) bonds.push([i, j]);
      }
    }
    return bonds;
  }

  function inferBondsSpatial(atoms) {
    var cells = new Map();
    var bonds = [];
    for (var i = 0; i < atoms.length; i++) {
      var key = bondCellKey(atoms[i]);
    ...
    for (var cellKey of cells.keys()) {
      var coords = cellKey.split(",").map(Number);
      var own = cells.get(cellKey);
      for (var dx = -1; dx <= 1; dx++) {
        for (var dy = -1; dy <= 1; dy++) {
          for (var dz = -1; dz <= 1; dz++) {
            var neighbor = cells.get((coords[0] + dx) + "," + (coords[1] + dy) + "," + (coords[2] + dz));
            if (!neighbor) continue;
            collectBondsFromBuckets(atoms, own, neighbor, bonds);
          }
        }
      }
    }

    bonds.sort(function (a, b) { return a[0] - b[0] || a[1] - b[1]; });
    return bonds;
  }

  function collectBondsFromBuckets(atoms, left, right, bonds) {
  ...
  function bondCellKey(atom) {
    return Math.floor(atom.x / maxBondDistance) + "," +
      Math.floor(atom.y / maxBondDistance) + "," +
      Math.floor(atom.z / maxBondDistance);
  }

  function isBonded(a, b) {
    var cutoff = ((radii[a.element] || 0.8) + (radii[b.element] || 0.8)) * bondRadiusScale;
    var limit = Math.min(cutoff, maxBondDistance);
    var dist = Math.hypot(a.x - b.x, a.y - b.y, a.z - b.z);
    return dist > minBondDistance && dist <= limit;
  }

  function centerAtoms(atoms, box) {
    box = box || bounds(atoms);
  ...
  function selectedIndexLookup(selected) {
    var lookup = {};
    for (var i = 0; i < selected.length; i++) {
      lookup[selected[i].index] = true;
    }
    return lookup;
  }
  ...
  function uiText(key, fallback, values) {
    var i18n = window.CP2KI18n;
    return i18n && i18n.text ? i18n.text(key, fallback, values) : fallback;
  }
